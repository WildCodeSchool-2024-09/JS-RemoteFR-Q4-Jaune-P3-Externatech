...
export default function SignUp({ isOpen, onClose }: LoginCompanyProps) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isCandidate) {
        await axios
          .post(`${import.meta.env.VITE_API_URL}/api/candidates`, dataToSend, {
    ...
    <dialog className="dialog_signup" open={isOpen}>
      {isSignIn ? (
        ...
        <form onSubmit={handleSubmit} className="opening_register">
          <h2>S'inscrire</h2>
          <p className="space_choice">Choisissez votre espace :</p>
          <button
            type="button"
            className="light-box"
            onClick={() => {
          ...
          {isCandidate ? (
            ...
            <div>
              ...
              <input
                ...
                id="siret"
                name="siret"
                value={credentials.siret}
                onChange={handleChange}
                placeholder="Numéro SIRET"
                required
              />
              <label htmlFor="company-logo">
                Logo de l'entreprise<span className="star"> *</span>
              </label>
              <input
                type="file"
                id="company-logo"
